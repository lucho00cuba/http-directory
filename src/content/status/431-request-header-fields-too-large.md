...
slug: "431-request-header-fields-too-large"
category: "client-error"
description: "Learn what the HTTP 431 Request Headers Fields Too Large status code means, when this error happens, how it relates to HTTP/2, and how to work around it in Nginx, Node.js, Apache, and Traefik."
updated: "2026-08-02"
created: "2026-08-02"
seeAlso:
  - "413-request-entity-too-large"
referenceUrl: "https://httpguides.com/status/431-request-header-fields-too-large"
---
...
Instead of returning the 431 status code when the cookie or header size is too large, Nginx typically responds with a [400 Bad Request](../400-bad-request) error. For oversized request lines, it returns a [414 Request-URI Too Large](../414-request-uri-too-long) error. To accommodate larger headers, you'll need to adjust a few directives. The main directive to focus on is <a href="https://nginx.org/en/docs/http/ngx_http_core_module.html#large_client_header_buffers" target="_blank" rel="noopener">`large_client_header_buffers`</a>:
...
Traefik enforces a single combined limit for the request line and headers through the <a href="https://doc.traefik.io/traefik/reference/install-configuration/entrypoints/" target="_blank" rel="noopener">`http.maxHeaderBytes`</a> entry point option. It defaults to 1 megabyte (`1048576` bytes) and, being built on Go's `net/http` package, returns `431` once that budget is exceeded.
...
There's no separate directive for cookie-related headers versus the rest of the request; raising `maxHeaderBytes` covers all of it.
