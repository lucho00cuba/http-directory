...
referenceUrl: "https://httpguides.com/status/101-switching-protocols"
---
...
If the server supports and is willing to talk using a new protocol, it will send back the `101 Switching Protocols` status code with the `Upgrade` header. If the server doesn't want or can't upgrade the connection, it will respond with the [`200 OK`](../200-ok) status code.
